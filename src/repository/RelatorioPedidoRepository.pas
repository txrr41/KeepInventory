unit RelatorioPedidoRepository;
...
type
  TfrmRelatorioDepreciacao = class
...
begin
  ...
  qryRelatorio.SQL.Text :=
    ...
    // Status Texto
    '  CASE ' +
    '    WHEN ((p.valor_aquisicao - COALESCE(p.valor_atual,0)) / NULLIF(p.valor_aquisicao,0)) * 100 >= 100 THEN ''J� DEPRECIADOS 100%'' ' +
    '    WHEN ((p.valor_aquisicao - COALESCE(p.valor_atual,0)) / NULLIF(p.valor_aquisicao,0)) * 100 >= 95 THEN ''URGENTE - PR�XIMOS 3 MESES (95%+)'' ' +
    '    WHEN ((p.valor_aquisicao - COALESCE(p.valor_atual,0)) / NULLIF(p.valor_aquisicao,0)) * 100 >= 85 THEN ''ATEN��O - 3 A 6 MESES (85-94%)'' ' +
  ...
  try
    ...
    on E: Exception do
    begin
      ShowMessage('Erro ao abrir query do relat�rio: ' + E.Message);
      raise;
    end;
  end;
